...
class PrimaryIPInitializer(BaseInitializer):
    def link_primary_ip(self, assets, asset_model):
        for params in assets:
            ...
            for assoc, details in OPTIONAL_ASSOCS.items():
                if assoc in params:
                    ...
                    try:
                        ...
                        self.log_warning(f"⚠️ IP Address '{query[field]}' not found")
            ...
            for field in primary_ip_fields:
                if getattr(asset, field) != params[field]:
                    ...
                    self.log(f"🔗 Define primary IP '{params[field].address}' on '{asset.name}'")
            asset.save()
    ...
    def load_data(self):
        ...
        if devices is not None:
            self.link_primary_ip(devices, Device)
        if virtual_machines is not None:
            self.link_primary_ip(virtual_machines, VirtualMachine)
